...
public class FieldData : BaseData
{
	...
	public FieldData(FieldDefinition field, bool ignorePrivate = true)
	{
		...
		this.Attributes = AttributeData.CreateArray(field.CustomAttributes);
	...
	public static List<FieldData> CreateArray(TypeDefinition type, bool recursive, bool isStatic, bool ignorePrivate = true)
	{
		if(!recursive)
		{
			List<FieldData> results = CreateArray(type.Fields, ignorePrivate);
	...
	/// <param name="ignorePrivate">Set to true to ignore this field if it's private within the declaring type</param>
	/// <returns>Returns the list of field informations</returns>
	public static List<FieldData> CreateArray(Collection<FieldDefinition> fields, bool ignorePrivate = true)
	{
		List<FieldData> results = new List<FieldData>();
		...
		foreach(FieldDefinition field in fields)
		{
			...
			FieldData info = new FieldData(field, ignorePrivate);
	...
	private static void RemoveUnwanted(List<FieldData> fields, bool isStatic, bool isOriginal)
	{
	...
	private static void RemoveDuplicates(List<FieldData> list, List<FieldData> listFields)
	{
		for(int i = list.Count - 1; i >= 0; i--)
		{
			foreach(FieldData field in listFields)
			{
