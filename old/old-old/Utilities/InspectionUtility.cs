...
public static class InspectionUtility
{
	#region Properties
	...
	public static List<string> GetGenericParametersAsStrings(string fullName)
	{
		...
		{
			results.Add(MakeNameFriendly(info.Name.Replace(",", ", ")));
		}
	...
	public static List<GenericParameterInspection> GetGenericParameters(string fullName)
	{
		...
		for(int i = curr; i < fullName.Length; i++)
		{
			...
			else if(fullName[i] == ',' && scope == 0)
			{
				...
				data.Name = MakeNameFriendly(data.Name);
				results.Add(data);
				curr = i + 1;
			}
		}
	...
	/// <summary>Finds if the attributes indicate an extension</summary>
	/// <param name="attributes">The attributes to look into</param>
	/// <returns>Returns true if the attributes indicate an extension by having the extension attribute</returns>
